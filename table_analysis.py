...
### ENS_MBIAS / ENS_VBIAS Density Map
mp.matshow(ENS_MBIAS.T[1:].T[1:]/ENS_VBIAS.T[1:].T[1:],cmap='spectral',vmin=-2,vmax=2)
cbar = mp.colorbar()
cbar.set_label('MBias / VBias',fontsize=12)
mp.grid()
mp.xticks([0,1,2,3,4,5],[5,10,15,25,50,100])
mp.yticks([0,1,2,3,4,5],[10,15,25,50,100,150])
mp.xlabel('Clusters per bin (Nclus)',fontsize=15)
mp.ylabel('Galaxies per Cluster (Ngal)',fontsize=15)
mp.tick_params(axis='x',labeltop='off',labelbottom='on')
mp.title("Ensemble Mass and Velocity Bias Comparison",fontsize=12)
